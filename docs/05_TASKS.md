...
기존 SVG 종가 차트를 `11_TECHNICAL_ANALYSIS_SPEC.md` 기준의 인터랙티브 차트로 교체했다. 모바일 실기기와 접근성 E2E 증적은 후속 검증으로 남긴다.
...
- [ ] 거래량 히스토그램과 Volume MA20 패널
- [ ] 볼린저 밴드 20·2 계산·API·오버레이
- [ ] 단순 RSI14를 Wilder RSI14로 버전 전환
- [ ] MA 골든·데드크로스, MACD 교차와 RSI 구간 진입·이탈 이벤트
- [ ] ATR14와 현재가 대비 ATR 비율 계산·보조 패널
- [ ] MA·거래량·볼린저 밴드·이벤트 표시/숨김과 RSI·MACD·ATR 패널 선택
- [ ] 고정 데이터셋 독립 교차검증과 경계·동일값·불완전 구간 테스트
- [ ] 데스크톱·모바일 가독성, 십자선 시점 정렬과 설정 유지 E2E 검증
...
`Step 2.1`의 다음 세로 흐름으로 기업 공식 IR/RSS 출처를 설정 기반 allowlist에 추가하고, Gemini 전문 분석의 위험 요인·근거 segment 출력과 청크별 토큰·비용 합산을 구현한다. 외부 키가 필요한 공급자는 로컬 환경변수로만 연결하고 키 없이도 DEMO와 HTTP Fixture 테스트가 동작하도록 유지한다.
