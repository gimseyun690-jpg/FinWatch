...
- [ ] `삼성`, `005930`, `Apple`, `AAPL` 검색이 이름·심볼·시장과 함께 올바르게 반환된다.
- [ ] symbol·이름 완전 일치가 부분 일치보다 먼저 나오고 pagination 중 중복·누락이 없다.
- [ ] 검색 요청은 외부 공급자를 호출하지 않고 공급자 장애 중에도 동작한다.
- [ ] 검색 결과 선택 후 URL, 현재가, 차트, 기술지표, 뉴스와 AI 기능이 모두 같은 종목을 사용한다.
- [ ] `METADATA_ONLY` 종목은 온디맨드 로딩 후 각 자원이 READY 또는 명확한 부분 실패 상태가 된다.
- [ ] 빠른 검색·종목 전환에서 이전 응답이 현재 선택을 덮어쓰지 않는다.
- [ ] 실시간 구독은 선택·관심·보유 종목으로 제한되고 해제·상한 정책이 동작한다.
- [ ] 동일 data-load 동시 요청은 외부 호출 한 번으로 합쳐진다.
- [ ] 동일 symbol의 다른 시장 후보가 안전하게 구분되고 canonical URL로 복원된다.
- [ ] 거래정지·상장폐지·마스터 빈 응답이 기존 사용자 데이터와 전체 catalog를 삭제하지 않는다.
- [ ] 데스크톱·390px 모바일·키보드·screen reader에서 검색과 선택이 가능하다.
- [ ] DEMO 4종목과 LIVE 전체 검색 범위를 UI와 발표에서 혼동하지 않는다.
