...
판정: `12_AI_TECHNICAL_EXPLANATION_SPEC.md`의 인수 조건을 모두 만족해야 한다. Mock 기반 MISS/HIT·무효화·근거·금지 출력 통합 테스트는 구현됐고 실제 Gemini 제한 smoke는 제출 전 별도 증적으로 남긴다.
...
판정: `13_AI_DAILY_CHANGE_BRIEFING_SPEC.md`의 인수 조건을 모두 만족해야 한다. 일봉 delta·서버 관점 매트릭스·근거·MISS/HIT·최신 조회의 Mock 통합 경로는 구현됐고 실제 Gemini smoke와 영상 증적은 제출 전 수행한다.
...
판정: `14_STOCK_DISCOVERY_SPEC.md`의 인수 조건을 모두 만족해야 한다. 로컬 검색·canonical URL·온디맨드 수집·동적 WebSocket 구독과 핵심 API/E2E는 구현됐으며 실제 LIVE 공급자 안정성은 제출 시연 환경에서 재확인한다.
