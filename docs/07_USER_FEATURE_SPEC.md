...
| 포트폴리오 CRUD·평가 | 구현 | 사용자 분리, 최신 가격 평가, KRW/USD 분리 합계와 화면 연결 |
| 가격 알림 CRUD·평가 | 구현 | ABOVE/BELOW 조건, 상태 전이와 최신 가격 평가 화면 연결 |
| 메인 대시보드 | 부분 구현 | 관심종목·종목 상세·AI·관리자 영역은 API 연결, 포트폴리오는 고정 예시값, 알림 영역 없음 |
...
5. 가격 응답에는 market_prices의 source와 asOf를 함께 제공한다.
...
상태: 구현. 사용자별 CRUD, 최신 저장 가격 평가와 KRW/USD 통화별 합계를 제공한다.
...
1. latestPrice는 조회 시점의 가장 최근 market_prices.close_price다.
...
상태: 구현. CRUD와 최신 저장 가격 기준 조건 충족 상태를 화면에 표시한다. 모바일 푸시·이메일 전송은 MVP 범위가 아니다.
...
1. 통화가 일치하는 가격만 비교한다.
2. 새 시장 가격이 수집된 직후 해당 종목의 ACTIVE 알림을 평가한다.
...
| AI 뉴스 | 화면에서 symbol=000660으로 고정 | 부분 구현 |
| 포트폴리오 | ₩28,940,000 등 고정 예시값 표시 | 미구현 데이터 |
| 가격 알림 | 영역 없음 | 미구현 |
| 관리자 AI 지표 | ADMIN에게만 표시하고 요약 후 새로고침 | 구현 |

고정 포트폴리오 숫자는 시안용이며 실제 데이터라는 표식을 해서는 안 된다. 포트폴리오 API 연결 전에는 “준비 중” 또는 명확한 “예시 데이터” 상태로 바꿔야 한다.
