  ...
  return (
    <article className="card daily-briefing-card" id="daily-briefing">
      ...
      {initialLoading && !briefing ? <div className="daily-briefing-empty" role="status" aria-busy="true">
        ...
        <details className="briefing-audit"><summary>AI 감사 정보</summary><p>{briefing.audit.modelName} · {briefing.audit.promptVersion}</p><p>{briefing.audit.calculationVersion} · {briefing.audit.briefingInputVersion}</p><p>근거 {briefing.audit.evidenceCount}개 · 제외 {briefing.audit.excludedContentCount}건 · {briefing.audit.responseTimeMs}ms</p><p>데이터 기준 {new Date(briefing.audit.latestRecordedAt).toLocaleString('ko-KR')}</p><p>생성 {new Date(briefing.audit.generatedAt).toLocaleString('ko-KR')} · {briefing.audit.cacheHit ? '캐시 적중 결과' : '신규 생성 결과'}</p></details>
        {error && <p className="request-error" role="alert">{error}</p>}
